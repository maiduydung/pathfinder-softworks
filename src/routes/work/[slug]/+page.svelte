<script lang="ts">
	...
	const images = $derived(
		study.images ??
			(study.image
				? [{ src: study.image, alt: `${study.title} screenshot` }]
				: []),
	);
...
<section class="mx-auto max-w-[1400px] px-6 pt-10 pb-8 lg:pt-16">
	<a
		href="/work"
		class="inline-flex items-center gap-1.5 text-sm text-light transition-colors hover:text-primary"
	>
		<svg
			class="h-4 w-4"
			fill="none"
	...
	<div class="mt-6 grid items-start gap-8 lg:grid-cols-2">
		...
		<div use:inview>
			...
			>
			<h1
				class="mt-2 flex items-center gap-3 font-bold text-3xl leading-snug tracking-tight text-foreground lg:text-4xl"
			>
				{study.title}
				{#if study.link}
					<a
						...
						class="text-light transition-colors hover:text-primary"
						title="Visit project"
					>
			...
			<p class="mt-3 text-base leading-relaxed text-light">
				{study.summary}
			</p>

			{#if study.founder}
				<div
					class="mt-4 flex flex-wrap items-center gap-1.5 text-sm text-light"
				>
					Founded by
					<a
						href="/team/{study.founder.slug}"
						class="font-medium text-primary hover:underline"
						>{study.founder.name}</a
					>
					<span
						class="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary"
						>{study.founder.credential}</span
					>
				</div>
			{/if}
			...
			<div class="mt-6 flex flex-wrap gap-4" use:inview={100}>
				{#each study.stats as stat}
					<div
						class="rounded-2xl border border-border bg-white px-5 py-3 text-center"
					>
						<p class="font-bold text-2xl tabular-nums text-primary">
							{stat.value}
						</p>
		...
		{#if images.length > 0}
			<div use:inview={100}>
					...
					<div class="relative w-full" style="aspect-ratio: 4/3;">
						{#each images as img, i}
							<img
								src={img.src}
								alt={img.alt}
								class="absolute inset-0 h-full w-full object-cover object-top transition-opacity duration-700 {i ===
								currentSlide
									? 'opacity-100'
									: 'opacity-0'}"
							/>
				...
				{#if images[currentSlide]}
					<p class="mt-2 text-center text-xs text-light">
	...
	<div class="mx-auto max-w-[1400px] px-6">
		<div class="grid gap-8 md:grid-cols-2">
			<div class="rounded-3xl border border-border p-6 lg:p-8" use:inview>
				...
				<h2 class="mt-2 font-bold text-xl text-foreground">
					What wasn't working
				</h2>
				<p class="mt-3 leading-relaxed text-light">{study.problem}</p>
			</div>
			<div
				class="rounded-3xl border border-border p-6 lg:p-8"
				use:inview={100}
			>
				<p
					class="text-xs font-bold uppercase tracking-widest text-primary"
	...
	<div class="mx-auto max-w-[1400px] px-6">
		...
		<div class="mt-4 grid gap-4 sm:grid-cols-2">
			{#each study.results as result, i}
				...
				<div
					class="flex items-center gap-4 rounded-2xl border border-border p-5"
					use:inview={i * 60}
				>
					<div
			...
			{#each study.tech as tech}
				{#if techUrls[tech]}
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						class="rounded-full border border-border px-4 py-1.5 text-sm font-medium text-foreground transition-colors hover:border-primary hover:text-primary"
						>{tech}</a
					>
				{:else}
					<span
						class="rounded-full border border-border px-4 py-1.5 text-sm font-medium text-foreground"
						>{tech}</span
					>
				{/if}
			{/each}
...
<!-- Next Project + CTA -->
<section
	class="border-t border-border bg-primary-light py-12 lg:py-16"
	use:inview
>
	<div class="mx-auto max-w-[1400px] px-6 text-center">
		...
		<a
			...
			class="mt-2 inline-flex items-center gap-2 font-bold text-2xl text-foreground transition-colors hover:text-primary lg:text-3xl"
		>
			{nextStudy.title}
			<svg
				class="h-5 w-5"
		...
		<h2
			class="font-bold text-2xl tracking-tight text-foreground lg:text-3xl"
		>
			Have a project in mind?
		</h2>
		<p class="mx-auto mt-2 max-w-lg text-light">
			Tell us the outcome you need. We'll tell you how we'd get there.
		</p>
		<div
