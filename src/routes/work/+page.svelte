...
<svelte:head>
	<title>Our Work - Pathfinder Softworks</title>
	<meta
		name="description"
...
<section class="mx-auto max-w-[1400px] px-6 pt-12 pb-8 lg:pt-20">
	<div class="max-w-2xl" use:inview>
		...
		<p class="mt-3 text-lg leading-relaxed text-light">
			Every project solved a real business problem and produced a
			measurable outcome.
		</p>
	</div>
</section>
			...
			>
				<div class="grid md:grid-cols-2">
					<div class="flex flex-col justify-center p-6 lg:p-10">
						...
						>
						<h2
							class="mt-2 text-2xl font-bold text-foreground transition-colors group-hover:text-primary lg:text-3xl"
						>
							{study.title}
						</h2>
						...
						<div class="mt-6 flex flex-wrap gap-1.5">
							{#each study.tech as tech}
								<span
									class="rounded-full bg-muted px-2.5 py-0.5 text-xs text-light"
									>{tech}</span
								>
							{/each}
						</div>
					...
					>
						...
						{:else}
							<div
								class="flex h-full min-h-[280px] flex-col items-center justify-center gap-3 p-8"
							>
								{#each study.stats as stat}
									<div
										class="w-full max-w-[260px] rounded-2xl border border-border bg-white px-5 py-4 text-center shadow-sm"
									>
										<p
