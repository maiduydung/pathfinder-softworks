...
<section class="mx-auto max-w-[1400px] px-6 pt-10 lg:pt-16">
	...
	<div class="mt-6 grid gap-8 lg:grid-cols-[280px_1fr]" use:inview>
		<!-- Left: photo + CTAs -->
		<div
			class="flex flex-col items-center lg:sticky lg:top-24 lg:self-start"
		>
			<img
				src={member.image}
				alt={member.name}
		...
		<div>
			<h1
				class="font-bold text-3xl tracking-tight text-foreground lg:text-4xl"
			>
				{member.name}
			</h1>
			<p class="mt-2 text-lg leading-relaxed text-light">
				{member.intro}
			</p>
			{#if isDuy}
				<p
					class="mt-2 inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-sm font-medium text-primary"
				>
					Selected to Sam Altman's Residency alongside elite global
					founders.
				</p>
			{/if}
...
<section class="mt-12 border-t border-border py-10 lg:mt-16 lg:py-14">
	<div class="mx-auto max-w-[1400px] px-6">
		<h2 class="mb-8 text-center font-bold text-xl text-foreground">
		...
		<div class="grid grid-cols-2 gap-x-4 gap-y-6 md:grid-cols-4">
			{#each otherMembers as other}
				<a href="/team/{other.slug}" class="group text-center">
					<div
						class="mx-auto h-20 w-20 overflow-hidden rounded-full border-2 border-transparent transition-all group-hover:border-primary/30 group-hover:shadow-md md:h-24 md:w-24"
					>
						<img
							src={other.image}
							alt={other.name}
