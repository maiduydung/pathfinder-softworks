		...
		try {
			const response = await fetch('https://api.web3forms.com/submit', {
				...
				body: JSON.stringify({
					...
					email,
					message,
					subject: `New message from ${name} — Pathfinder Softworks`
				})
			});
			...
			<div>
				<form onsubmit={handleSubmit} class="space-y-4">
					...
					<button
						...
						class="w-full rounded-lg bg-primary px-6 py-3 font-medium text-white transition-colors hover:bg-primary-hover disabled:opacity-50"
					>
						{#if status === 'sending'}
							Sending...
						{:else if status === 'sent'}
